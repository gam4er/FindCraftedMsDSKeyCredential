...
";
        ...
        static void Add(string target, string fqdn, string dc, string path, string password)
        {
            ...
            keyCredential = new KeyCredential(cert, guid, "CN=LOGAN_HOWARD,OU=Tier 1,DC=gam,DC=click" /*targetObject.Properties["distinguishedName"][0].ToString()*/, DateTime.Now);
            ...
            try
            {
                ...
                Console.WriteLine("generated KeyCredential attribute: {0}", keyCredential.ToDNWithBinary());
                Console.WriteLine("KeyCredential attribute in AD    : {0}", (string)targetObject.Properties["msDS-KeyCredentialLink"][0]);
                targetObject.Properties["msDS-KeyCredentialLink"][0] = keyCredential.ToDNWithBinary();
